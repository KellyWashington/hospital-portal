// Oasis Attendance Automation Chrome Extension
// Fully translates Python Excel processor to JavaScript using SheetJS & ExcelJS

// State variables to store parsed input data
...
// Setup upload zone drag-and-drop animations
...
function handleAttendanceFile(file) {
  log(`Loading attendance file: ${file.name} (${formatBytes(file.size)})...`);
  ...
  reader.onload = function(e) {
    try {
      ...
      attendanceData = getSheetRows(sheet);
      
      if (attendanceData.length > 0) {
        document.getElementById('attendance-name').innerText = file.name;
        document.getElementById('attendance-info').innerText = `${attendanceData.length} records parsed`;
        attendanceZone.classList.add('loaded');
        document.getElementById('attendance-status').innerText = 'Loaded';
        log(`Successfully parsed ${attendanceData.length} rows from attendance sheet.`, 'success');
      } else {
        throw new Error('Attendance sheet is empty or headers are missing.');
      }
      checkReady();
    } catch (err) {
      log(`Error parsing attendance file: ${err.message}`, 'error');
      attendanceData = null;
      attendanceZone.classList.remove('loaded');
      document.getElementById('attendance-status').innerText = 'Error';
    }
  };
...
function handleShiftsFile(file) {
  log(`Loading shifts file: ${file.name} (${formatBytes(file.size)})...`);
  const reader = new FileReader();
  reader.onload = function(e) {
    try {
      const data = new Uint8Array(e.target.result);
      ...
      // We parse three sheets: Employees, Department_Rules, Shifts_Config
      // Let's identify the sheet names (handling variations like 'Shifts' or 'Employees')
      let employeesSheetName = workbook.SheetNames.find(n => n === 'Employees' || n === 'Shifts');
      let rulesSheetName = workbook.SheetNames.find(n => n === 'Department_Rules');
      let configSheetName = workbook.SheetNames.find(n => n === 'Shifts_Config');
      
      if (!employeesSheetName) {
        // Fallback to first sheet
        employeesSheetName = workbook.SheetNames[0];
      }
      
      const empSheet = workbook.Sheets[employeesSheetName];
      const employees = getSheetRows(empSheet);
      
      let rules = [];
      if (rulesSheetName) {
        rules = getSheetRows(workbook.Sheets[rulesSheetName]);
      }
      
      let configs = [];
      if (configSheetName) {
        configs = getSheetRows(workbook.Sheets[configSheetName]);
      }
      ...
      document.getElementById('shifts-info').innerText = `${employees.length} employees, ${rules.length} dept rules, ${configs.length} configs`;
      shiftsZone.classList.add('loaded');
      document.getElementById('shifts-status').innerText = 'Loaded';
      
      log(`Successfully parsed shifts sheet. Employees: ${employees.length}. Department Rules: ${rules.length}. Configs: ${configs.length}.`, 'success');
      checkReady();
    } catch (err) {
      log(`Error parsing shifts file: ${err.message}`, 'error');
      shiftsData = null;
      shiftsZone.classList.remove('loaded');
      document.getElementById('shifts-status').innerText = 'Error';
    }
  };
  reader.readAsArrayBuffer(file);
}
...
// Helper: parse worksheet cells to structured JS rows
...
// -------------------------------------------------------------
// CORE PROCESSING ENGINE (Translated from Python)
// -------------------------------------------------------------
...
generateBtn.addEventListener('click', async () => {
  ...
  // Yield execution to UI thread briefly so logger updates
  await new Promise(r => setTimeout(r, 50));
  
  try {
    await processAndDownload();
    log("Report generated and downloaded successfully!", "success");
  } catch (err) {
...
async function processAndDownload() {
  ...
  rawPunches = rawPunches.filter(p => {
    ...
    if (seenPunches.has(key)) return false;
    seenPunches.add(key);
    return true;
  });
  ...
  scheduledSessions.forEach(s => {
    ...
    if (s.MissingCheckin === 'Yes') {
      ...
      // Base lateness label extraction
  ...
  // 5. Excel Generation & Styling (ExcelJS)
  ...
  [ws_status, ws_emp, ws_sess].forEach(ws => {
    ws.columns.forEach(col => {
      ...
      // For Status Summary, skip row 4 & 5 to avoid KPI card cell widths affecting the column widths
...
// Helper: formats size in bytes to human-readable
